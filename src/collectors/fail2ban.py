...
class Fail2banCollector(BaseCollector):
    ...
    def collect(self) -> Dict[str, Any]:
        ...
        try:
            t1 = time.time()
            status_result = subprocess.run([FAIL2BAN_CLIENT, "status"], capture_output=True, text=True, timeout=10)
    ...
    def _get_jail_info(self, jail_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run([FAIL2BAN_CLIENT, "status", jail_name], capture_output=True, text=True, timeout=5)
    ...
    def _get_jail_bantime(self, jail_name: str) -> int:
        ...
        try:
            result = subprocess.run(
                [FAIL2BAN_CLIENT, "get", jail_name, "bantime"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
    ...
    def _get_recent_unbans(
        self, limit: int = UNBAN_HISTORY_LIMIT, exclude_ips: Optional[Set[str]] = None
    ...
    def _get_slow_bots_from_cache(self, exclude_ips: Optional[Set[str]] = None) -> Optional[Dict[str, Any]]:
    ...
    def _get_ip_data(self, ip: str) -> Dict[str, Any]:
        ...
        info = {"country": "Unknown", "org": "Unknown", "attempts": 0, "last_updated": 0}
    ...
    def _fetch_geo_data(self, ip: str) -> Dict[str, str]:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=2) as response:
                ...
                return {"country": data.get("country", "Unknown"), "org": data.get("org", "Unknown")}
        except Exception as e:
    ...
    def _get_traefik_target_for_ip(
        self, ip: str, log_path: str = "/home/app_data/docker/traefik/logs/access.log"
    ) -> str:
        ...
        try:
            ...
            grep_proc = subprocess.Popen([GREP, "-a", ip, log_path], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
            tail_proc = subprocess.Popen(
                [TAIL, "-n", "5"], stdin=grep_proc.stdout, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True
            )
    ...
    def ban_ip(self, ip: str, jail: str = "recidive") -> bool:
        ...
        try:
            ...
            if jail == "recidive":
                ...
                t1 = time.time()
                result = subprocess.run(
                    [FAIL2BAN_CLIENT, "set", jail, "bantime", str(RECIDIVE_BANTIME)], timeout=5, capture_output=True
                )
            ...
            t2 = time.time()
            result = subprocess.run(
                [FAIL2BAN_CLIENT, "set", jail, "banip", ip], check=True, timeout=5, capture_output=True
            )
    ...
    def migrate_recidive_bans(self) -> Tuple[int, int]:
        ...
        try:
            subprocess.run(
                [FAIL2BAN_CLIENT, "set", "recidive", "bantime", str(RECIDIVE_BANTIME)], timeout=5, capture_output=True
            )
        except Exception as e:
        ...
        try:
            result = subprocess.run([FAIL2BAN_CLIENT, "status", "recidive"], capture_output=True, text=True, timeout=10)
            if result.returncode != 0:
                return (0, 0)
            ...
            for ip in banned_ips:
                try:
                    # Unban
                    subprocess.run([FAIL2BAN_CLIENT, "set", "recidive", "unbanip", ip], timeout=5, capture_output=True)
                    # Re-ban with new bantime
                    subprocess.run([FAIL2BAN_CLIENT, "set", "recidive", "banip", ip], timeout=5, capture_output=True)
                    success += 1
    ...
    def run_analysis(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["sudo", "python3", script_path, "--json"], capture_output=True, text=True, timeout=120
            )
