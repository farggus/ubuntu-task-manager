...
class AttacksDatabase:
    ...
    def migrate_from_cache(self, cache_dir: Path) -> Dict[str, int]:
        """
        Migrate data from old cache files.
        
        Args:
            cache_dir: Path to cache directory with old files
            
        Returns:
            Dict with migration statistics
        """
        stats = {"ips_migrated": 0, "geo_migrated": 0, "whitelist_migrated": 0}
        
        with self._lock:
            # Migrate bans_db.json
            bans_db_path = cache_dir / "bans_db.json"
            if bans_db_path.exists():
                try:
                    with open(bans_db_path, 'r', encoding='utf-8') as f:
                        bans_db = json.load(f)
                    
                    for ip, data in bans_db.items():
                        if ip not in self._data["ips"]:
                            self._data["ips"][ip] = self._create_empty_ip_record()
                        
                        record = self._data["ips"][ip]
                        record["attempts"]["total"] = data.get("attempts", 0)
                        record["geo"]["country"] = data.get("country")
                        record["geo"]["org"] = data.get("org")
                        
                        if data.get("last_updated"):
                            record["last_updated"] = data["last_updated"]
                        
                        stats["ips_migrated"] += 1
                    
                    logger.info(f"Migrated {stats['ips_migrated']} IPs from bans_db.json")
                except Exception as e:
                    logger.error(f"Failed to migrate bans_db.json: {e}")
            
            # Migrate ip_geo_cache.json
            geo_cache_path = cache_dir / "ip_geo_cache.json"
            if geo_cache_path.exists():
                try:
                    ...
                    for ip, data in geo_cache.items():
                        if ip in self._data["ips"]:
                            self._data["ips"][ip]["geo"]["country"] = data.get("country")
                            self._data["ips"][ip]["geo"]["org"] = data.get("org")
                            stats["geo_migrated"] += 1
                    
                    logger.info(f"Migrated geo data for {stats['geo_migrated']} IPs")
                except Exception as e:
                    logger.error(f"Failed to migrate ip_geo_cache.json: {e}")
            
            # Migrate whitelist.json
            whitelist_path = cache_dir / "whitelist.json"
            if whitelist_path.exists():
                try:
                    ...
                    for ip in whitelist:
                        self.add_to_whitelist(ip, reason="Migrated from cache", added_by="migration")
                        stats["whitelist_migrated"] += 1
                    
                    logger.info(f"Migrated {stats['whitelist_migrated']} whitelist entries")
                except Exception as e:
                    logger.error(f"Failed to migrate whitelist.json: {e}")
            
            # Recalculate stats and scores after migration
            self.recalculate_stats()
            self.recalculate_danger_scores()
            self._data["metadata"]["last_full_sync"] = _now_iso()
            self._dirty = True
