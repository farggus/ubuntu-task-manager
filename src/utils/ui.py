...
def generate_braille_sparkline(data_list: List[float], width: int = 25, max_val: float = 0.0) -> str:
    ...
    def get_char(v1, v2):
        ...
        for i in range(l_level): left_dots[3-i] = 1
        for i in range(r_level): right_dots[3-i] = 1
        
        # Map dots to Braille bitmask
        # 1:0x01, 2:0x02, 3:0x04, 4:0x08, 5:0x10, 6:0x20, 7:0x40, 8:0x80
        mask = 0
        if left_dots[0]: mask |= 0x01
        if left_dots[1]: mask |= 0x02
        if left_dots[2]: mask |= 0x04
        if left_dots[3]: mask |= 0x40
        
        if right_dots[0]: mask |= 0x08
        if right_dots[1]: mask |= 0x10
        if right_dots[2]: mask |= 0x20
        if right_dots[3]: mask |= 0x80
